...
window.addEventListener('load', () => {
    document.querySelector('#toggle-chat-pane').addEventListener('click', () => {
        let chatElem = document.querySelector('#chat-pane');
        let mainSecElem = document.querySelector('#main-section');

        if (chatElem.classList.contains('chat-opened')) {
            chatElem.setAttribute('hidden', true);
            mainSecElem.classList.replace('col-md-9', 'col-md-12');
            chatElem.classList.remove('chat-opened');
        } else {
            chatElem.removeAttribute('hidden');
            mainSecElem.classList.replace('col-md-12', 'col-md-9');
    ...
    document.getElementById('local').addEventListener('click', () => {
        if (!document.pictureInPictureElement) {
            document.getElementById('local').requestPictureInPicture().catch(console.error);
        } else {
    ...
    document.addEventListener('click', (e) => {
        if (e.target.classList.contains('expand-remote-video')) {
            helpers.maximiseStream(e);
        } else if (e.target.classList.contains('mute-remote-mic')) {
